...
MAX_CONCURRENT_TASKS = 10  # Limit concurrent tasks to avoid overloading
...
async def fetch_json(session, url, params):
    """Fetch JSON data from the given URL with provided parameters."""
    try:
        async with session.get(url, params=params) as response:
            response.raise_for_status()
            return await response.json()
    except aiohttp.ClientError as e:
        logger.error(f"Request failed: {e} - URL: {url} - Params: {params}")
...
async def get_erc1155_tokens(session, address):
    """Retrieve ERC1155 tokens associated with a given address."""
    params = {
        "module": "account",
        "action": "tokennfttx",
        "address": address,
...
async def process_address(session, address, file, semaphore, i, total_addresses, start_time):
    ...
    async with semaphore:  # Limit the number of concurrent tasks
        ...
        if tokens:
            async with asyncio.Lock():  # Ensure file access is synchronized
